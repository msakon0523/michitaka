...
def read_images(data_dir, use_labels=False, generate_seq=False):
    files = os.listdir(data_dir)#データの読み込み
    files_dir = [f for f in files if os.path.isdir(os.path.join(data_dir, f))]#ファイルの中身を保存image_gaol000000とか
    files_dir = [data_dir + f for f in files_dir]#ファイルの名前をくっつけるimage/env_image/image_goal0000000
    dir_num = len(files_dir)
    images = []
   
    for file_dir in files_dir:
        #for image in glob.glob(file_dir + "/*.jpg")[1300:1305]:
        for image in glob.glob(file_dir + "/*.jpg"):#指定したものを保存（今回はjpgのついているもの），正規表現と*が使える
            images.append(image)
    #images = glob.glob(data_dir + "*")
    random.shuffle(images)
    images.sort()#学習の時はランダムにするのでこれは非表示設定，テストの時はソートにする
    #images = images[:100]
    sample_image = cv2.imread(images[0])#RGBで読み込みと保存，シェイプは64*64*3でした
    data = np.empty([len(images), sample_image.shape[0], sample_image.shape[1], sample_image.shape[2]])#今回は35200(1100mai*32datas),64,64,3
    labels = []
    for i, image in enumerate(images):#ナンバリングと引数をセットにするin_enumerate関数
        ...
        #data.append(rgb_image)
            ...
            #print image
        data[i, :, :, :] = rgb_image#dataに1~35200までナンバリングしながら保存
        if use_labels:
