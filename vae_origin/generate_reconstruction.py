...
flags.DEFINE_string("data_dir", "./image/test_images/", "Directory of training data")
...
def main(_):
    ...
    data, data_num, data_shape, seq_num = read_images(FLAGS.data_dir, generate_seq=True)
    seq_length = int(data_num / seq_num)
    input_placeholder, target_placeholder = make_placeholder(data_shape)
    #channel_list = [data_shape[2], 500, 5]
    channel_list = [data_shape[2], 32, 32, 100, 20]
    ...
    with tf.Session() as sess:
        ...
        for i in range(seq_num):
            ...
            if not os.path.exists(FLAGS.save_dir + "/reconstruction"):
                os.mkdir(FLAGS.save_dir + "/reconstruction")
            reconstructed_images = denormalize(reconstructed_images, min_out=0., max_out=1.,scale=1.).astype(np.uint8)
            for j, rec_img in enumerate(reconstructed_images):
                cv2.imwrite(FLAGS.save_dir + "/reconstruction/reconstruction{:0>6d}".format(j) + ".jpg", rec_img)
    ...
    for i in range(n):
        ...
        plt.imshow(cv2.cvtColor(reconstructed_images[i], cv2.COLOR_BGR2RGB))
