...
class VAE:
    def __init__(self, ch_list=[3,32,32,16,8], k_h=5, k_w=5, stddev=0.01):
        ...
        with tf.variable_scope("ae"):
            #encoder
            self.w_enc1 = tf.Variable(tf.truncated_normal([28 * 28 * self.ch_list[0], self.ch_list[1]], stddev=stddev))#[2352,32]範囲0.01
            self.b_enc1 = tf.Variable(tf.zeros([self.ch_list[1]]))#32
            self.w_enc2 = tf.Variable(tf.truncated_normal([self.ch_list[1], self.ch_list[1]], stddev=stddev))#[32,32]
            self.b_enc2 = tf.Variable(tf.zeros([self.ch_list[1]]))#32
            self.w_enc3_mu = tf.Variable(tf.truncated_normal([self.ch_list[1], self.ch_list[2]], stddev=stddev))#[32,32]
            self.b_enc3_mu = tf.Variable(tf.zeros([self.ch_list[2]]))#32
            self.w_enc3_ln_var = tf.Variable(tf.truncated_normal([self.ch_list[1], self.ch_list[2]], stddev=stddev))#[32,32]
            self.b_enc3_ln_var = tf.Variable(tf.zeros([self.ch_list[2]]))#32
            
            #decoder
            self.w_dec3 = tf.Variable(tf.truncated_normal([self.ch_list[2], self.ch_list[1]],stddev=stddev))#[32,32]
            self.b_dec3 = tf.Variable(tf.zeros([self.ch_list[1]]))#32
            self.w_dec2 = tf.Variable(tf.truncated_normal([self.ch_list[1], self.ch_list[1]],stddev=stddev))#32
            self.b_dec2 = tf.Variable(tf.zeros([self.ch_list[1]]))#32
            self.w_dec1 = tf.Variable(tf.truncated_normal([self.ch_list[1], 28 * 28 * self.ch_list[0]], stddev=stddev))#[32,2353]
            self.b_dec1 = tf.Variable(tf.zeros([28 * 28 * self.ch_list[0]]))#2352

    def __call__(self, x, batch_size, train=True):

        #Encoder1 (500)
        inputs = tf.reshape(x, [-1, self.ch_list[0] * 28 * 28])#-1逆数
        inter_enc1 = tf.matmul(inputs, self.w_enc1) + self.b_enc1
        h_enc1 = tf.nn.softplus(inter_enc1)#活性化関数max(0,x)+log(1+e^-|x|)
